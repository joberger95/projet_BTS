...
void main()
{
    H_Prog = 0x10; M_Prog= 0x19; S_Prog= 0x00;// Heures minutes secondes
                                             // d'initialisation de l'horloge
    config();                                //Appel le sous-programme
    delay_ms(1000);                          //Délai 1s
    Lcd_Cmd(_LCD_CLEAR);
    while(1)                                 //Boucle infinie
    {
     Test_Complet();                         // Transmissions (Série et GSM)
     Lecture_Heure();                        //Lecture de l'heure
     Aff_Heure();                            //Affichage de l'heure
     delay_ms(100);                          //Délai 100ms
     Lecture_TempHum();                      //Lecture temp et hum
     Aff_TempHum();                          //Affichage temp et hum
     delay_ms(100);                          //Délai 100ms
     Vpile();                                //Mesure et affichage tension de
                                             //pile
    }
}
/*******************************SOUS-PROGRAMMES********************************/
...
I2C1_Repeated_Start();                //Repeated start
  ...
  reponseH=I2C1_Rd(0);
  I2C1_Stop();
  reponseS=reponseS & 0x7F;           //Forcage à 0 du Bit 8
}                                     //(Marche arrêt RTC = HTR)
void Aff_Heure()
{
  ...
  Lcd_Chr(1,1,DizHeure);              //Affichage de la
...
void Aff_TempHum()
{
HumTotale=HumMSB*256;                 //Mise en place de la formule
HumTotale=HumTotale+HumLSB;           //donnée par le constructeur
humcalcul=HumTotale;
humcalcul=humcalcul/16382;
humcalcul=humcalcul*100;

 Resteh = humcalcul;                  // Reste = Partie entière
 centh=Resteh/100;                    //Décodage pour l'affichage de la valeur
 Resteh= Resteh -cent*100;
 dizh= Resteh /10;
 unith= Resteh % 10;

 if(cent==1) Lcd_chr(2,2,centh+0x30); //Affichage de la valeur
...
tempcalcul= TempTotale;               //Mise en place de la formule
tempcalcul=tempcalcul/16382;          //donnée par le constructeur
tempcalcul=tempcalcul*165;
tempcalcul=tempcalcul-40;

 Reste = tempcalcul;                  // Reste = Partie entière
 cent=Reste/100;                      //Décodage pour l'affichage de la valeur
 Reste= Reste -cent*100;
 diz= Reste /10;
 unit= Reste % 10;

 if(cent==1) Lcd_chr(2,12,cent+0x30); //Affichage de la valeur
